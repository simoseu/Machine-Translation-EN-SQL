...
for e in range(len(train_sentences)):
  train_set.append({"input_ids": tokenizer(train_sentences[e], truncation = False, padding=True)["input_ids"], "labels": tokenizer(train_query_flatted[e], truncation = False, padding=True)["input_ids"]})


test_set = list()
for e in range(len(test_sentences)):
  test_set.append({"input_ids": tokenizer(test_sentences[e], truncation = False, padding=True)["input_ids"], "labels": tokenizer(test_query_flatted[e], truncation = False, padding=True)["input_ids"]})
...
for i in range(len(test_sentences)):
    preds = model.generate(tokenizer(test_sentences[i], return_tensors = "pt")["input_ids"], max_length = 150)
    predictions = [tokenizer.decode(t, skip_special_tokens=True) for t in preds]
    metrica = (meteor.compute(predictions = [predictions[0]], references = [test_query_flatted[i]]))
